...
NAN_METHOD(MakeKeypair) {
	...
	if (ed25519_extend(seed, privateKeyData)) {
		return Nan::ThrowError("Not a valid BIP32-Ed25519 seed");
	}
	...
	ed25519_publickey(privateKeyData, publicKeyData);
...
/*NAN_METHOD(Sign) {
	...
	if ((Buffer::HasInstance(info[1])) && (Buffer::Length(info[1]->ToObject()) == 32)) {
		...
		ed25519_sign_keypair(publicKeyData, privateKeyData);
	...
	ed25519_sign(signatureMessageData, &sigLen, messageData, messageLen, privateKey);
...
/*NAN_METHOD(Verify) {
	...
	info.GetReturnValue().Set(ed25519_sign_verify(signatureData, messageData, messageLen, publicKeyData) == 0);
}
*/
